define([
		...
		if (SKBeInstant.isWLA()) {
			...
			for (i = 0; i < revealConfigurations.length; i++) {
				...
				for(j=0; j< prizeTable.length; j++){
					tBody += '<tr><td nowrap="nowrap">' + (j+1) + '</td><td nowrap="nowrap">' + SKBeInstant.formatCurrency(prizeTable[j].prize).formattedAmount + '</td></tr>';                
				}
				tBody += '</tbody></table>';               
			}        
    ...
    msgBus.subscribe('startReveallAll', disableConsole);
    msgBus.subscribe('allRevealed', enableConsole);
